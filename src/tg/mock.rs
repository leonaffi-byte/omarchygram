...
fn msg(id: i32, chat_id: i64, chat_title: &str, sender: &str, text: &str, ts: DateTime<Local>, outgoing: bool) -> Msg {
    Msg {
        id,
        chat_id,
...
fn fmt_msg(id: i32, chat_id: i64, chat_title: &str, sender: &str, markdown: &str, ts: DateTime<Local>, outgoing: bool) -> Msg {
    let (text, spans) = parse_markdown(markdown);
    Msg { text, spans, markdown: markdown.to_string(), ..msg(id, chat_id, chat_title, sender, "", ts, outgoing) }
}

fn group_msg(id: i32, sender: &str, sender_id: i64, text: &str, ts: DateTime<Local>) -> Msg {
    Msg { sender_id: Some(sender_id), ..fmt_msg(id, 4, "Arch Linux ARM", sender, text, ts, false) }
}
...
impl MockState {
    fn new() -> Self {
        ...
        history.insert(1, vec![
            msg(101, 1, "Marta", "Marta", "did you see the fog this morning", t(95), false),
            msg(102, 1, "Marta", "You", "yeah, rode through it on the way to work", t(93), true),
            Msg { media: Some(MediaKind::Photo), photo_size: Some((1280, 800)), ..msg(103, 1, "Marta", "Marta", "", t(91), false) },
            Msg {
                reactions: vec![Reaction { emoji: "👍".into(), count: 1, chosen: false }],
                ..msg(104, 1, "Marta", "Marta", "send pics next time", t(90), false)
            },
            Msg { reply_to: Some(103), ..msg(105, 1, "Marta", "You", "that one's from the pass", t(88), true) },
            Msg { media: Some(MediaKind::Sticker), sticker_emoji: Some("😀".into()), ..msg(106, 1, "Marta", "Marta", "", t(85), false) },
            fmt_msg(107, 1, "Marta", "Marta", "also are we still on for **thursday**?", t(12), false),
        ]);
        ...
        history.insert(3, vec![
            Msg { media: Some(MediaKind::Voice), duration: Some(12), ..msg(301, 3, "Mom", "Mom", "", t(1502), false) },
            msg(302, 3, "Mom", "Mom", "call me when you're free", t(1500), false),
            msg(303, 3, "Mom", "You", "will do, after dinner", t(1440), true),
        ]);
        history.insert(4, vec![
            group_msg(401, "Robin", 4001, "linux 7.1.9-arch1-2 has landed in core", t(2100)),
            group_msg(402, "Ada", 4002, "anyone tried it on the rpi5 yet?", t(2050)),
            Msg {
                spans: vec![Span { start: 0, end: 6, kind: SpanKind::Mention(4001) }],
            ...
            Msg { media: Some(MediaKind::Gif), doc_name: Some("it-works.mp4".into()), ..group_msg(405, "Robin", 4001, "", t(1980)) },
        ...
        history.insert(ME_ID, vec![
            msg(601, ME_ID, "Saved Messages", "You", "todo: renew the domain", t(4000), true),
            Msg {
                media: Some(MediaKind::Document),
        ...
        history.insert(7, vec![
            msg(701, 7, "Old project", "Sam", "archiving this, thanks everyone", t(20_000), false),
            Msg {
        ...
        deleted.insert(2, vec![Msg { deleted: true, ..msg(205, 2, "Deni", "Deni", "never mind, wrong chat", t(329), false) }]);
        ...
        let contacts = vec![
            Contact { user_id: 1, name: "Marta".into(), username: "marta_k".into(), phone: "+1 555 0142".into(), presence: Presence::Online, has_photo: true },
            Contact { user_id: 2, name: "Deni".into(), username: "deni".into(), phone: String::new(), presence: Presence::LastSeen(t(140)), has_photo: false },
            Contact { user_id: 3, name: "Mom".into(), username: String::new(), phone: "+1 555 0101".into(), presence: Presence::LastWeek, has_photo: false },
            Contact { user_id: 5005, name: "Sam Rivera".into(), username: "samr".into(), phone: "+1 555 0177".into(), presence: Presence::Recently, has_photo: false },
            Contact { user_id: 5006, name: "Yuki Tanaka".into(), username: String::new(), phone: "+81 90 0000 0000".into(), presence: Presence::LastMonth, has_photo: false },
        ];
        let folders = vec![
            Folder { id: 1, title: "Work".into(), chats: vec![2, 4] },
            Folder { id: 2, title: "Family".into(), chats: vec![3] },
        ];
        MockState {
    ...
    fn chat_title(&self, chat_id: i64) -> String {
        self.chats.get(&chat_id).map(|c| c.title.clone()).unwrap_or_else(|| "Someone".into())
    }
    ...
    fn summary(&self, c: &MockChat) -> ChatSummary {
        ...
        let last_sender = match (c.kind, last) {
            (ChatKind::Group, Some(m)) => {
                if m.outgoing { "You".to_string() } else { m.sender.split_whitespace().next().unwrap_or("").to_string() }
            }
            _ => String::new(),
        };
        ChatSummary {
    ...
    fn find(&self, chat_id: i64, msg_id: i32) -> Option<Msg> {
        self.history
            ...
            .or_else(|| self.deleted.get(&chat_id).and_then(|v| v.iter().find(|m| m.id == msg_id)))
            .cloned()
    }
    ...
    // Marta goes offline/online every 20s so presence rendering is exercised.
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                online = !online;
                let presence = if online { Presence::Online } else { Presence::LastSeen(Local::now()) };
                if let Some(c) = st.lock().unwrap().chats.get_mut(&1) {
                    c.presence = presence;
                }
                if events.send(Event::Presence { user_id: 1, presence }).await.is_err() {
                    break;
                }
            }
        });
...
fn schedule_read(st: Arc<Mutex<MockState>>, events: &async_channel::Sender<Event>, chat_id: i64, msg_id: i32) {
    let events = events.clone();
    tokio::spawn(async move {
        ...
        let _ = events.send(Event::ReadOutbox { chat_id, max_id: msg_id }).await;
    });
}
...
async fn handle(cmd: Command, st: Arc<Mutex<MockState>>, events: async_channel::Sender<Event>) {
    if !matches!(
        cmd,
        Command::Start(_) | Command::SubmitCredentials { .. } | Command::SubmitPhone(..) | Command::SubmitCode(..) | Command::SubmitPassword(..)
    ) {
    ...
    match cmd {
            ...
            let mut result = match before_id {
                ...
                Some(before) => {
                    ...
                    if chat_id == 1 && msgs.first().is_some_and(|m| m.id == before) {
                        vec![
                            msg(90, 1, "Marta", "Marta", "older message from last week", t(10000), false),
                            msg(91, 1, "Marta", "You", "yep, scroll-back works", t(9990), true),
                        ]
                    } else {
                        vec![]
                    }
        ...
        Command::DownloadAvatar { chat_id, respond } => {
            let has = st.lock().unwrap().chats.get(&chat_id).is_some_and(|c| c.has_photo)
                || st.lock().unwrap().contacts.iter().any(|c| c.user_id == chat_id && c.has_photo);
            tokio::spawn(async move {
                tokio::time::sleep(std::time::Duration::from_millis(150)).await;
                let _ = respond.send(Ok(if has { sample_image_n(chat_id as usize) } else { None }));
            });
        }
        Command::SendText { chat_id, text, reply_to, respond } => {
            let sent = {
                ...
                let sent = Msg { reply_to, ..fmt_msg(st.next_id, chat_id, &title, "You", &text, t(0), true) };
        ...
        Command::ForwardMessages { from_chat, ids, to_chat, respond } => {
            let out = {
                ...
                let originals: Vec<Msg> = ids.iter().filter_map(|&id| st.find(from_chat, id)).collect();
                let mut out = Vec::new();
                for o in originals {
                    st.next_id += 1;
        ...
        Command::SearchGlobal { query, respond } => {
            ...
            let mut hits: Vec<Msg> = if q.is_empty() {
                ...
                st.all_messages_newest_first().into_iter().filter(|m| contains_ci(&m.text, q)).collect()
            };
            hits.truncate(50);
            let _ = respond.send(Ok(hits));
        }
        Command::SearchChats { query, respond } => {
            ...
            let mut out = Vec::new();
            if !q.is_empty() {
                for c in st.contacts.iter().filter(|c| !st.chats.contains_key(&c.user_id)) {
        ...
        Command::GetPinnedMessage { chat_id, respond } => {
            let st = st.lock().unwrap();
            let m = st.pinned_msg.get(&chat_id).and_then(|&id| st.find(chat_id, id));
        ...
        Command::SendReaction { chat_id, msg_id, emoji, respond } => {
            let changed = {
                let mut st = st.lock().unwrap();
                st.history.get_mut(&chat_id).and_then(|msgs| msgs.iter_mut().find(|m| m.id == msg_id)).map(|m| {
                    ...
                    if let Some(e) = emoji {
                        match m.reactions.iter_mut().find(|r| r.emoji == e) {
                            ...
                            None => m.reactions.push(Reaction { emoji: e, count: 1, chosen: true }),
                        }
                    }
                    m.clone()
                })
        ...
        Command::GetAvailableReactions(tx) => {
            let _ = tx.send(Ok(["👍", "❤️", "🔥", "😂", "😮", "😢", "👏", "🎉", "🙏", "🤔"].iter().map(|s| s.to_string()).collect()));
        }
        Command::SetPinned { chat_id, pinned, respond } => {
            let ok = st.lock().unwrap().chats.get_mut(&chat_id).map(|c| c.pinned = pinned).is_some();
            let _ = events.send(Event::DialogsChanged).await;
            let _ = respond.send(if ok { Ok(()) } else { Err(format!("unknown chat {chat_id}")) });
        }
        Command::SetMuted { chat_id, mode, respond } => {
            let ok = st.lock().unwrap().chats.get_mut(&chat_id).map(|c| c.muted = mode != MuteMode::Unmute).is_some();
            let _ = events.send(Event::DialogsChanged).await;
            let _ = respond.send(if ok { Ok(()) } else { Err(format!("unknown chat {chat_id}")) });
        }
        Command::SetArchived { chat_id, archived, respond } => {
            let ok = st.lock().unwrap().chats.get_mut(&chat_id).map(|c| c.archived = archived).is_some();
            let _ = events.send(Event::DialogsChanged).await;
            let _ = respond.send(if ok { Ok(()) } else { Err(format!("unknown chat {chat_id}")) });
        }
        Command::MarkUnread { chat_id, unread, respond } => {
            let ok = st.lock().unwrap().chats.get_mut(&chat_id).map(|c| c.unread_mark = unread).is_some();
            let _ = events.send(Event::DialogsChanged).await;
            let _ = respond.send(if ok { Ok(()) } else { Err(format!("unknown chat {chat_id}")) });
        }
        ...
        Command::SaveDraft { chat_id, text, reply_to, respond } => {
            ...
            let _keep = reply_to;
            let ok = st.lock().unwrap().chats.get_mut(&chat_id).map(|c| c.draft = text).is_some();
            let _ = respond.send(if ok { Ok(()) } else { Err(format!("unknown chat {chat_id}")) });
        }
        ...
        Command::GetMembers { chat_id, offset, limit, respond } => {
            ...
            let named = [("Robin", 4001, MemberRole::Creator, Presence::Online), ("Ada", 4002, MemberRole::Admin, Presence::Recently), ("Kai", 4003, MemberRole::Member, Presence::LastWeek)];
            let mut all: Vec<Member> = named
                .iter()
                .map(|(n, id, role, p)| Member { user_id: *id, name: n.to_string(), username: n.to_lowercase(), presence: *p, role: *role })
                .collect();
            for i in all.len() as i32..total {
                all.push(Member { user_id: 5000 + i as i64, name: format!("Member {}", i + 1), presence: Presence::LongAgo, ..Member::default() });
            }
            let out: Vec<Member> = all.into_iter().skip(offset.max(0) as usize).take(limit.max(0) as usize).collect();
            let _ = respond.send(Ok(out));
        }
                ...
                .map(|v| {
                    v.iter()
                        ...
                        .filter(|m| match kind {
                            ...
                            SharedKind::Files => matches!(m.media, Some(MediaKind::Document | MediaKind::Video)),
                            SharedKind::Links => m.webpage.is_some() || m.spans.iter().any(|s| matches!(s.kind, SpanKind::Link(_))),
        ...
        Command::OpenUser { user_id, respond } => {
            let r = {
                ...
                if let Some(c) = st.chats.get(&user_id) {
                    ...
                    st.chats.insert(7007, MockChat { username: "omarchy_bot".into(), ..chat(7007, "Omarchy Bot", ChatKind::Bot) });
        ...
        Command::CreateGroup { title, user_ids, respond } => {
            let r = {
                ...
                if title.is_empty() {
                    ...
                    st.next_id += 1;
                    let first = msg(st.next_id, id, &title, "You", "You created the group", t(0), true);
        ...
        Command::GetStickerPacks(tx) => {
            let _ = tx.send(Ok(vec![
                StickerPack { id: "recent".into(), title: "Recent".into(), count: 3 },
                StickerPack { id: "favorites".into(), title: "Favorites".into(), count: 1 },
                StickerPack { id: "9100".into(), title: "Omarchy".into(), count: 4 },
            ]));
        }
        Command::GetStickers { pack_id, respond } => {
            ...
            let out: Vec<Sticker> = list
                .into_iter()
                .map(|id| Sticker { id, emoji: sticker_emoji(id).into(), animated: id == 9104 })
                .collect();
            let _ = respond.send(Ok(out));
        }
        Command::DownloadSticker { sticker_id, respond } => {
            tokio::spawn(async move {
                ...
                let _ = respond.send(Ok(if sticker_id == 9104 { None } else { sample_image_n(sticker_id as usize) }));
            });
        }
        Command::GetSavedGifs(tx) => {
            let _ = tx.send(Ok(vec![Gif { id: 9201, width: 320, height: 240 }, Gif { id: 9202, width: 480, height: 270 }]));
        }
...
fn schedule_reply(
    ...
    tokio::spawn(async move {
        ...
        let (title, kind) = {
            let st = st.lock().unwrap();
            (st.chat_title(chat_id), st.chats.get(&chat_id).map(|c| c.kind).unwrap_or_default())
        };
        let (name, sender_id) = if kind == ChatKind::Group { ("Robin".to_string(), 4001) } else { (title.clone(), chat_id) };
        let _ = events.send(Event::Typing { chat_id, name: name.clone() }).await;
        tokio::time::sleep(std::time::Duration::from_millis(1200)).await;
        let reply = Msg { sender_id: Some(sender_id), ..msg(reply_id, chat_id, &title, &name, "(mock reply) got it", t(0), false) };
        {
            let mut st = st.lock().unwrap();
