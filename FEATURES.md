...
IN PROGRESS (2026-09-03): backend contract + real implementation + video-note recorder merged; 6B (cards, polls), 6C (poll/location dialogs, send later, scheduled), 6D (animated stickers) MERGED; 6A (playback) and 6E (bots, forums) in verification; 6F next; 6G calls: spike done, deferred (see the Wave 6 section at the end). Secret chats are
